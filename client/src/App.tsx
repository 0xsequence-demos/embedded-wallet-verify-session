...
function VerifyWallet({ wallet }: any) {
  const [verificationResult, setVerificationResult] = useState<WalletInfo | null>(null);
  ...
  return (
    <div style={{
      ...
      <Card>
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <Button label="Verify Wallet on Backend 🔗" onClick={() => verifyWalletBackend()} />
      </Card>
      ...
      {error && (
        ...
        {verificationResult && (
          <Card style={{ backgroundColor: '#e8f5e9', color: '#2e7d32' }}>
            ...
            <p>Issued At: {new Date(verificationResult.iat * 1000).toLocaleString()}</p>
            <p>Expires At: {new Date(verificationResult.exp * 1000).toLocaleString()}</p>
          </Card>
        )}
